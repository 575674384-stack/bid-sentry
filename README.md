...
- 制作人、最后修改人、组织、应用、时间、文档标识等受支持字段会重置为合法、非空的随机值，而不是清空。
- 预览和执行使用同一随机计划；输入文件字节、大小、时间和文件身份保持不变。
- 输出是新文件，例如 `投标文件_sanitized.docx`，并附带不包含原值/随机值的 JSON/HTML 验证报告。
...
- 先填写投标单位、统一社会信用代码、地址、法定代表人/授权代表、联系人、联系方式、项目/标段和编制日期等信息，再确认填充计划。
...
应用内三类流程分别是：选择文件 → 选择输出/报告目录 → 预览或确认 → 执行 → 查看验证结果。原文件不会被覆盖。
